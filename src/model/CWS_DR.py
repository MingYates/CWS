...
class CWS_DR:
    ...
    def build(self):
        ...
        with tf.name_scope('bilstm_m'):
            ...
            self.cell_fw_att = tf.nn.rnn_cell.BasicLSTMCell(outdim_lstm_att//2, name='cell_fw_m')
            self.cell_bw_att = tf.nn.rnn_cell.BasicLSTMCell(outdim_lstm_att//2, name='cell_bw_m')
            _bilstm_m, _ = tf.nn.bidirectional_dynamic_rnn(self.cell_fw_att, self.cell_bw_att, inputs=self.in_m, dtype=tf.float32,
                                                           sequence_length=tf.reshape(self.in_len, shape=[-1]))
            _bilstm_m = tf.concat(_bilstm_m, axis=-1)
            _bilstm_m = tf.reshape(_bilstm_m, shape=[-1, outdim_lstm_att])

        with tf.name_scope('concat'):
            _bilstm = tf.concat([_bilstm_c, _bilstm_m], axis=-1)
